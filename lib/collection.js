...
// item data. It plays together with CollectionSubset and adds itself as an
// observer to each of its members.
class CollectionBase {
  constructor(members) {
    ...
    let self = this
    this._members.forEach(i => i.addObserver(self))
  }
  ...
  filter(...xs) { return this._members.filter(...xs) }
  reduce(...xs) { return this._members.reduce(...xs) }
...
class LoveInterestCollectionSubset extends CollectionSubset {
  ...
  post_add(item) {
    let domobj = item.make_big_card()
    this._item_domobj_map.set(item, domobj)
    this._domobj.appendChild(domobj)
  }
